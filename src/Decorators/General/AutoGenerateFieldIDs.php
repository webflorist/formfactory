...
class AutoGenerateFieldIDs extends Decorator
{
    ...
    public static function decorate(Element $element)
    {
        if (!$element->attributes->isSet('id')) {
            ...
            // Autogenerated IDs always start with formID.
            $fieldId = $formBuilderService->openForm->attributes->getValue('id');

            // The field-element containing the field-name is always $element,
            // except with option-elements, where we use the currently open select-element.
            $fieldElement = $element;
            if (is_a($element, OptionElement::class)) {
            ...
            if (is_a($element, RadioInputElement::class) || is_a($element, OptionElement::class)) {
                $fieldId .= '_' . $element->attributes->getValue('value');
            }
